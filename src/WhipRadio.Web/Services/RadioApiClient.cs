...
public class RadioApiClient(HttpClient http, IHttpClientFactory httpClientFactory, ILogger<RadioApiClient> logger)
{
    ...
    public async Task<bool> SetGuestVoiceFxAsync(Guid guestId, string? voiceFx, CancellationToken ct = default)
    {
        using var response = await http.PutAsJsonAsync(
            $"/api/guests/{guestId}/voice-fx", new UpdateGuestVoiceFxRequestDto(voiceFx), ct);
        return response.IsSuccessStatusCode;
    }
    ...
    public async Task<bool> RescanArchiveAsync(CancellationToken ct = default)
    {
        using var response = await http.PostAsync("/api/archive/rescan", null, ct);
        return response.IsSuccessStatusCode;
    }
    ...
    public async Task<bool> AcceptArchiveCandidateAsync(Guid trackId, Guid candidateId, CancellationToken ct = default)
    {
        using var response = await http.PostAsync($"/api/archive/{trackId}/candidates/{candidateId}/accept", null, ct);
        return response.IsSuccessStatusCode;
    }

    public async Task<bool> RejectArchiveCandidateAsync(Guid trackId, Guid candidateId, CancellationToken ct = default)
    {
        using var response = await http.PostAsync($"/api/archive/{trackId}/candidates/{candidateId}/reject", null, ct);
        return response.IsSuccessStatusCode;
    }

    public async Task<bool> KeepArchiveTrackLocalAsync(Guid trackId, CancellationToken ct = default)
    {
        using var response = await http.PostAsync($"/api/archive/{trackId}/keep-local", null, ct);
        return response.IsSuccessStatusCode;
    }
    ...
    public async Task<DeleteTrackResult> DeleteArchiveTrackAsync(Guid id, CancellationToken ct = default)
    {
        using var response = await http.DeleteAsync($"/api/archive/{id}", ct);
        if (response.StatusCode == HttpStatusCode.NoContent)
        {
            return new DeleteTrackResult(Deleted: true, Deferred: false, Error: null);
        }
