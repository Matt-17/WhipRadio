...
- God-class decomposition candidates (large, untested): `RadioApiClient`. `ChatActionExecutor` is split across partials. `AudioMixerEngine` is decomposed: pure math extracted to Core (`SourceScheduler`, `TransitionRealizer`, `FadeRealizer`, alongside `MixerCore`/`MixPlanner`) and DB access to `MixerSessionStore`; the engine keeps only the clock loop and encoder feed. `StudioCoordinator` is decomposed: it composes `StudioBookingRegistry` (in-memory bookings + GPU leases under one gate), `StudioEndpointProber` (connection test / runtime probes), and `StudioPendingOperationsTracker` (studios-page pending ops); callers keep using the coordinator façade.
